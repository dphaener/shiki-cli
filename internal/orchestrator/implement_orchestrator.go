...
func (o *ImplementOrchestrator) Initialize() error {
	...
	logger.log("========== INITIALIZING IMPLEMENT AGENT ==========")
	...
	o.implementInstructions = agentCfg.SystemPrompt
	o.instructionsSent = false

	logger.log("========== IMPLEMENT INSTRUCTIONS ==========")
	logger.log("%s", o.implementInstructions)
	logger.log("========== END IMPLEMENT INSTRUCTIONS ==========")
	...
	if err := o.agent.Start(o.ctx, o.apiKey, agentCfg.ID); err != nil {
		return fmt.Errorf("failed to start implement agent: %w", err)
	}
...
func (o *ImplementOrchestrator) SendMessage(userMessage string) (<-chan MessageUpdate, <-chan error) {
	...
	go func() {
		...
		// If this is the first message, prepend the implement instructions
		if !o.instructionsSent && o.implementInstructions != "" {
			logger.log("Prepending implement instructions to first message")
			messageToSend = o.implementInstructions + "\n\n---\n\n## User Request\n\n" + userMessage
		...
		for attempt := 0; attempt <= maxRetries; attempt++ {
			...
			// Create query context with timeout for this attempt - longer for implementation
			queryCtx, cancel := context.WithTimeout(o.ctx, 10*time.Minute)
