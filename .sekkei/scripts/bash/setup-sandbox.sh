...
Bootstrap a fresh Specify sandbox using the local Sekkei CLI checkout.
...
Options:
  ...
  --skip-install          Skip reinstalling sekkei-cli via uv tool install
...
if [[ "$SKIP_INSTALL" != true ]]; then
    echo "Installing sekkei-cli (version $CLI_VERSION) from $REPO_ROOT"
    set +e
    uv tool install --from "$REPO_ROOT" sekkei-cli --force
    install_status=$?
    set -e
    if [[ $install_status -ne 0 ]]; then
        if command -v sekkei >/dev/null 2>&1; then
            echo "Warning: sekkei-cli install failed (exit $install_status); existing installation will be reused." >&2
        else
            echo "Error: sekkei-cli install failed (exit $install_status) and no existing sekkei command was found. Re-run with --skip-install after installing manually." >&2
            exit 1
        fi
    fi
else
    echo "Skipping sekkei-cli installation per request"
fi

export SEKKEI_TEMPLATE_ROOT="$REPO_ROOT"

if ! command -v sekkei >/dev/null 2>&1; then
    echo "Error: sekkei executable not found in PATH. Install sekkei-cli or rerun without --skip-install." >&2
    exit 1
fi
...
sekkei init "$DEST_ABS" --ai "$NORMALISED_AGENTS" --script "$SCRIPT_TYPE"

echo
echo "Sandbox ready at $DEST_ABS"
echo "Template root used: $SEKKEI_TEMPLATE_ROOT"
