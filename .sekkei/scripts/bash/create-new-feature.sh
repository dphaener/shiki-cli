...
if [ -z "$FEATURE_DESCRIPTION" ]; then
    cat >&2 <<'EOF'
[sekkei] Error: Feature description missing.
...
find_repo_root() {
    local dir="$1"
    while [ "$dir" != "/" ]; do
        if [ -d "$dir/.git" ] || [ -d "$dir/.sekkei" ]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
...
SPECS_DIR_BASE="$REPO_ROOT/sekkei-specs"
HIGHEST=0
            ...
            if [ -d "$WORKTREE_PATH" ]; then
                if git -C "$WORKTREE_PATH" rev-parse --show-toplevel >/dev/null 2>&1; then
                    ...
                    if [ "$CURRENT_WORKTREE_BRANCH" = "$BRANCH_NAME" ] || [ "$CURRENT_WORKTREE_BRANCH" = "HEAD" ]; then
                        ...
                        >&2 echo "[sekkei] Warning: Reusing existing worktree at $WORKTREE_PATH for $BRANCH_NAME."
                    else
                        >&2 echo "[sekkei] Warning: Existing worktree at $WORKTREE_PATH is checked out to $CURRENT_WORKTREE_BRANCH; skipping worktree creation."
                    fi
                else
                    >&2 echo "[sekkei] Warning: Worktree path $WORKTREE_PATH exists but is not a git worktree; skipping worktree creation."
                fi
            else
                if git worktree add "$WORKTREE_PATH" -b "$BRANCH_NAME" >/dev/null 2>&1; then
                    ...
                    >&2 echo "[sekkei] Warning: Unable to create git worktree for $BRANCH_NAME; falling back to in-place checkout."
                fi
            fi
        else
            >&2 echo "[sekkei] Warning: Git worktree command unavailable; falling back to in-place checkout."
        fi
    fi
    ...
    if [ "$WORKTREE_CREATED" != "true" ]; then
        if git show-ref --verify --quiet "refs/heads/$BRANCH_NAME"; then
            if ! git checkout "$BRANCH_NAME"; then
                >&2 echo "[sekkei] Error: Failed to check out existing branch $BRANCH_NAME"
                exit 1
            fi
        else
            if ! git checkout -b "$BRANCH_NAME"; then
                >&2 echo "[sekkei] Error: Failed to create branch $BRANCH_NAME"
                exit 1
            fi
        fi
    fi
else
    >&2 echo "[sekkei] Warning: Git repository not detected; skipped branch creation for $BRANCH_NAME"
fi

REPO_ROOT="$TARGET_ROOT"
cd "$REPO_ROOT"

SPECS_DIR="$REPO_ROOT/sekkei-specs"
mkdir -p "$SPECS_DIR"
...
SPEC_TEMPLATE_CANDIDATES=(
    ...
    "$REPO_ROOT/.sekkei/templates/spec-template.md"
...
if [ -n "$TEMPLATE" ]; then
    ...
    echo "[sekkei] Copied spec template from $TEMPLATE"
else
    echo "[sekkei] Warning: Spec template not found for active mission; creating empty spec.md"
    touch "$SPEC_FILE"
fi
