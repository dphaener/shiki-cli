...
scripts:
  sh: .sekkei/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks
  ps: .sekkei/scripts/powershell/Check-Prerequisites.ps1 -Json -RequireTasks -IncludeTasks
---
...
1. **Verify worktree context**:
   ...
   - If that directory is present and you are not already inside it, `cd` into the worktree before proceeding.
    - When inspecting git status or listing files, always reference the worktree paths (for example, `sekkei-specs/<feature>/...` inside `.worktrees/<feature>/`).
...
3. **MANDATORY: Initialize Task Workflow** ⚠️ BLOCKING STEP
   ...
   **For EACH task you will implement**:
   ...
   a. **Move task prompt to doing lane**:
      ...
      .sekkei/scripts/bash/tasks-move-to-lane.sh FEATURE-SLUG TXXX doing \
        --shell-pid "$SHELL_PID" \
        --agent "claude" \
        --note "Started implementation"
      ```
      > Windows users: run `.sekkei/scripts/powershell/tasks-move-to-lane.ps1` with the same arguments.
   ...
   **If validation fails**: STOP and fix the workflow before implementing.
   (Optional) Run `.sekkei/scripts/bash/validate-task-workflow.sh TXXX FEATURE_DIR` for automated checks.
...
8. Progress tracking and error handling:
   ...
   - **After completing each task**:
     ...
     ```bash
     .sekkei/scripts/bash/tasks-move-to-lane.sh FEATURE-SLUG TXXX for_review \
       --shell-pid "$SHELL_PID" \
       --agent "claude" \
...
1. **START**: `planned/` → `doing/`
   - `.sekkei/scripts/bash/tasks-move-to-lane.sh FEATURE-SLUG WPID doing --note "Started implementation"`
...
3. **COMPLETE**: `doing/` → `for_review/`
   ...
   - `.sekkei/scripts/bash/tasks-move-to-lane.sh FEATURE-SLUG WPID for_review --note "Ready for review"`
