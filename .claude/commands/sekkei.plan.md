...
scripts:
  sh: .sekkei/scripts/bash/setup-plan.sh --json
  ps: .sekkei/scripts/powershell/Setup-Plan.ps1 -Json
agent_scripts:
  sh: .sekkei/scripts/bash/update-agent-context.sh __AGENT__
  ps: .sekkei/scripts/powershell/Update-Agent-Context.ps1 -AgentType __AGENT__
---
...
3. **Load context**: Read FEATURE_SPEC and `.sekkei/memory/constitution.md`. Load IMPL_PLAN template (already copied).
...
4. **Execute plan workflow**: Follow the structure in IMPL_PLAN template, using the validated planning answers as ground truth:
   ...
   - Phase 0: Run `sekkei research` (or `/sekkei.research`) to scaffold research.md, data-model.md, and research CSV logs, then populate findings using the validated planning answers
...
> Kick off this phase by running `sekkei research` to scaffold the mission-specific files listed below. Then use the checklist to enrich each artifact with the clarifications uncovered during planning.
