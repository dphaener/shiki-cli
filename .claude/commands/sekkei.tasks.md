...
Breaks down the implementation plan into work packages and subtasks.
...
1. **Setup**: Run `{SCRIPT}` from repo root and capture `FEATURE_DIR` plus `AVAILABLE_DOCS`. All paths must be absolute.
   ...
   **CRITICAL**: The script returns JSON with `FEATURE_DIR` as an ABSOLUTE path (e.g., `/Users/robert/Code/new_specify/sekkei-specs/001-feature-name`).

   **YOU MUST USE THIS PATH** for ALL subsequent file operations. Example:
   ```
   FEATURE_DIR = "/Users/robert/Code/new_specify/sekkei-specs/001-a-simple-hello"
...
5. **Write `tasks.md`** using `.sekkei/templates/tasks-template.md`:
...
6. **Generate prompt files (one per work package)**:
   ...
   - For each work package:
     ...
     - Use `.sekkei/templates/task-template.md` to capture:
...
7. **Report**: Provide a concise outcome summary:
  ...
  - Prompt generation stats (files written, directory structure, any skipped items with rationale)
   - Next suggested command (e.g., `/sekkei.analyze` or `/sekkei.implement`)
