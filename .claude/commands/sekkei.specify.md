...
scripts:
  sh: .sekkei/scripts/bash/create-new-feature.sh --json "{ARGS}"
  ps: .sekkei/scripts/powershell/create-new-feature.ps1 -Json "{ARGS}"
---

## Usage

```
/sekkei.specify <feature description>
```

Creates a new feature specification through an interactive discovery process.
...
The text the user typed after `/sekkei.specify` in the triggering message **is** the initial feature description. Capture it verbatim, but treat it only as a starting point for discovery—not the final truth. Your job is to interrogate the request, surface gaps, and co-create a complete specification with the user.
...
2. When discovery is complete and the intent summary **and title** are confirmed, run the script `{SCRIPT}` from repo root, inserting `--feature-name "<Friendly Title>"` (replace the quoted text with the confirmed title) immediately before the feature description argument. For example:
   ...
   - **bash/zsh**: `.sekkei/scripts/bash/create-new-feature.sh --json --feature-name "Checkout Upsell Flow" "$ARGUMENTS"`
   - **PowerShell**: `.sekkei/scripts/powershell/create-new-feature.ps1 -Json -FeatureName "Checkout Upsell Flow" "$ARGUMENTS"`

   Parse its JSON output for `BRANCH_NAME`, `SPEC_FILE`, `FEATURE_NUM`, `FRIENDLY_NAME`, and `WORKTREE_PATH`. All file paths must be absolute.

   **IMPORTANT** You must only ever run this script once. The JSON is provided in the terminal as output - always refer to it to get the actual content you're looking for.

3. **Handle Worktree Context Switching** (T019-T021):

   a. **Attempt Automatic Directory Change** (T019):
      - If `WORKTREE_PATH` is provided in the JSON output (non-empty string), the script created a worktree
      - Attempt to change to the worktree directory: `cd {WORKTREE_PATH}`
      - Note: This may fail in some AI agent contexts due to shell limitations

   b. **Verify Working Directory** (T020):
      - After attempting `cd`, verify your current working directory
      - Check if you're in the correct location by comparing paths
      - Expected location: the value from `WORKTREE_PATH` field

   c. **Provide User Instructions if Needed** (T021):
      - If the automatic `cd` succeeded and verification passes: proceed to step 4
      - If the automatic `cd` failed or working directory doesn't match `WORKTREE_PATH`:
         1. Output clear instructions for the user:
            ```
            ⚠️ WORKTREE CONTEXT REQUIRED

            The script created a feature worktree, but I cannot change directories automatically.

            Please change to the worktree directory manually:

                cd {WORKTREE_PATH}

            After changing directories, you can:
            - Continue this conversation to generate the specification
            - Re-run /sekkei.specify if needed

            The worktree is required because all feature files must be created in the
            isolated worktree, not in the main repository.
            ```
         2. STOP execution and wait for user to change directories
         3. Do NOT proceed with spec generation until user confirms they are in the correct directory

   d. **Final Verification Before File Operations**:
      - Before writing any files in step 5, verify once more that you're in the correct directory
      - If verification fails, abort with error message and instructions
...
7. **Specification Quality Validation**: After writing the initial spec, validate it against quality criteria:
   ...
   a. **Create Spec Quality Checklist**: Generate a checklist file at `FEATURE_DIR/checklists/requirements.md` using the checklist template structure with these validation items:
      ...
      **Feature**: [Link to spec.md]
      ...
      - Items marked incomplete require spec updates before `/sekkei.clarify` or `/sekkei.plan`
      ```
...
6. Report completion with branch name, spec file path, checklist results, and readiness for the next phase (`/sekkei.clarify` or `/sekkei.plan`).
